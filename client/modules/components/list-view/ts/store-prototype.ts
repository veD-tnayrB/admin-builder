...
export /*bundle*/ abstract class StoreListView extends ReactiveModel<StoreListView> {
	...
	constructor({ collection, id }: { collection: Collection; id: string }) {
		...
		this.#propertiesDisplaying = JSON.parse(
	...
	#getPlugins = () => {
		this.#plugins.forEach((plugin: string) => {
			...
			methods.forEach(
				(method: string) => (this[method] = instance[method])
			);
		});
	};
		...
		try {
			...
			const isSpecificFilter =
				...
				Object.keys(search).some(
					element => !this.generalFilters.includes(element)
				);

			if (!isSpecificFilter) {
				this.generalFilters.forEach(item => {
					const value =
						typeof search === 'string' ? search : search[item];
					if (!value) return;
